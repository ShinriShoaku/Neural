...
====================================
Smoke test 2-stage buat adapter system_core:
    1. Stage 1 (SYSTEM_STAGE1_PROMPT) -> dapat task_category
    2. Kalau category == "ambiguous_negative" -> langsung output None,
       TIDAK panggil model lagi.
       Selain itu -> Stage 2 (prompt sesuai category) -> action/target/parameters

Cara pakai:
    python smoke_test_system_hierarchical.py \\
        --base-model-dir ./models/Qwen3.5-0.8B \\
        --adapter-dir ./adapters/system_core_v1
"""
...
from system_chatml import SYSTEM_STAGE1_PROMPT, STAGE2_PROMPTS, CATEGORIES

# (kalimat, expected_category, expected_action, expected_target_value_or_None, expected_parameters)
# Kalimat BARU, bukan turunan persis dari generate_system_full.py -- tes generalisasi asli.
TEST_CASES = [
    ("Nyalain aplikasi telegram.", "application", "launch", "telegram", {}),
    ("Tutup vscode dong.", "application", "close", "vscode", {}),
    ("Cek apakah spotify masih nyala.", "application", "check_app_status", "spotify", {}),

    ("Bunuh proses nginx yang lagi jalan.", "process", "kill_process", "nginx", {}),
    ("Tampilin daftar proses yang aktif sekarang.", "process", "list_processes", None, {}),

    ("Bikinin folder baru namanya Proyek Baru.", "filesystem", "create_folder", "Proyek Baru", {}),
    ("Hapus file resume.pdf.", "filesystem", "delete_file", "resume.pdf", {}),
    ("Geser catatan.txt ke folder Arsip Lama.", "filesystem", "move_file", "catatan.txt", {"destination": "Arsip Lama"}),

    ("Coba jalanin `docker ps` di terminal.", "shell", "run_command", "docker ps", {}),

    ("Aktifin mikrofon laptop.", "hardware", "enable_device", "mikrofon", {}),
    ("Matiin fitur airplane mode.", "hardware", "disable_device", "airplane mode", {}),

    ("Diemin suaranya bentar.", "audio", "mute", None, {}),
    ("Gedein volume dikit.", "audio", "volume_up", None, {"amount": 10}),
    ("Atur volume ke 35%.", "audio", "set_volume", None, {"level": 35}),

    ("Redupin layar dikit.", "display", "brightness_down", None, {}),
    ("Pasang brightness di 60%.", "display", "set_brightness", None, {"level": 60}),

    ("Sambungin ke wifi Kos_Elit.", "network", "connect_wifi", "Kos_Elit", {}),
    ("Cek status wifi sekarang.", "network", "check_connection", None, {}),

    ("Berapa persen sisa baterai?", "system_query", "check_resource", "baterai", {}),
    ("Gimana status penggunaan disk?", "system_query", "check_resource", "disk", {}),

    ("Ceritain dongeng soal naga.", "ambiguous_negative", None, None, {}),
    ("Kok bisa error terus ya ini.", "ambiguous_negative", None, None, {}),
]
...
def run_pipeline(model, tokenizer, text: str, max_new_tokens: int) -> dict:
    problems = []
    raw1 = generate(model, tokenizer, SYSTEM_STAGE1_PROMPT, text, max_new_tokens=30)
    ...
    if category == "ambiguous_negative":
        return {"category": category, "action": None, "target_value": None, "parameters": {},
                "raw1": raw1, "raw2": None, "problems": problems}
...
def evaluate(result, expected_category, expected_action, expected_target_value, expected_parameters):
    ...
    if result["category"] != expected_category:
        ...
        return problems  # kalau category sudah salah, jangan cek lebih jauh (nggak fair)
    if expected_action is None:
        return problems  # ambiguous_negative, tidak ada action yang dicek
...
def main() -> None:
    ...
    for cat, statuses in by_cat.items():
        ...
        print(f"  {cat:22s} {p}/{len(statuses)}")
