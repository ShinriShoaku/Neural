...
=====================
Versi smoke test untuk adapter router hasil build_router_simple.py
(skema output {"segments": [{"domain","text"}]}, TANPA confidence/id/
overlap_hint).

Beda penting dari smoke_test_batch.py yang lama: script itu CUMA cek
format valid (JSON valid + domain masuk enum) -- TIDAK pernah cek apakah
domain yang dipilih itu BENAR secara semantik. Jadi kategori seperti
compound_command bisa "PASS" walau domainnya salah total. Script ini
menutup gap itu dengan expected_domains eksplisit per kalimat.

Cara pakai:
    python smoke_test_simple.py \\
        --base-model-dir ./models/Qwen3.5-0.8B \\
        --adapter-dir ./adapters/router_core_v4
"""
...
# (kalimat, task_category, expected_domains_berurutan)
# expected_domains dipakai buat cek KETEPATAN, bukan cuma validitas format.
TEST_CASES = [
    ...
    ("Lanjutin lagi yang tadi ya.", "context_dependent", None),  # domain tergantung context, tidak dicek ketat
...
def evaluate_case(parsed, expected_domains):
    ...
    # Kadang model keluar list mentah [...] langsung, bukan {"segments": [...]}
    if isinstance(parsed, list):
        segments = parsed
        problems.append("output list mentah, bukan {\"segments\": [...]}")
    elif isinstance(parsed, dict):
        segments = parsed.get("segments")
    else:
        return [f"tipe output tidak dikenal: {type(parsed).__name__}"]
    if not isinstance(segments, list) or len(segments) == 0:
        problems.append("Field 'segments' kosong / bukan list")
        return problems
    actual_domains = [seg.get("domain") if isinstance(seg, dict) else None for seg in segments]
...
def actual_domains_for_print(parsed):
    if isinstance(parsed, list):
        segs = parsed
    elif isinstance(parsed, dict):
        ...
        return []
    return [s.get("domain") if isinstance(s, dict) else s for s in segs]
...
def main() -> None:
    ...
    for text, category, expected_domains in TEST_CASES:
        ...
        if problems:
            ...
            print(f"    raw: {raw[:200]}")
        if parsed:
            doms = actual_domains_for_print(parsed)
